...
namespace AncientBastionRemake {
    ...
    StartSpot@[] spots = {
        ...
        StartSpot(AIFloat3( 2800, 0,  5400), AiRole::FRONT, false), 
        StartSpot(AIFloat3( 4100, 0,  6600), AiRole::FRONT, false),

		StartSpot(AIFloat3( 10700, 0,  4300), AiRole::FRONT, false)
    };

    // Base per-map unit limits
    dictionary mapUnitLimits; // add per-map unit restrictions here if needed

    MapConfig config = MapConfig("Ancient Bastion Remake", mapUnitLimits, spots, getFactoryWeights());
